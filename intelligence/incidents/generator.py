...
class IncidentGenerator:
    ...
    def cleanup_stale_tracks(self, active_track_ids: set) -> None:
        """
        Remove tracks from the buffer that are no longer active to prevent memory leaks.
        """
        stale = [tid for tid in self._event_buffer if tid not in active_track_ids]
        for tid in stale:
            del self._event_buffer[tid]
